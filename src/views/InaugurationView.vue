<template>
  <div class="relative min-h-screen w-full bg-black font-mono text-gray-100 flex flex-col justify-between overflow-x-hidden select-none">
    ...
    <main class="relative z-10 flex-1 flex flex-col items-center justify-center p-4 sm:p-6 my-auto">
      ...
      <!-- CINEMATIC ANIMATION STAGE (When Authorized) -->
      <div v-if="isAuthorized" class="w-full">
        <CinematicInauguration :qr-url="inaugurationUrl" />
      </div>

      <!-- AUTHORIZATION FORM STAGE (Before Authorization) -->
      <div v-else class="w-full max-w-xl bg-neutral-950/90 border border-red-700/80 p-6 sm:p-8 rounded-xl shadow-[0_0_50px_rgba(255,0,50,0.4)] backdrop-blur-xl space-y-6">
